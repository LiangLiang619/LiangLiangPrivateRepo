---
title: "✅ 亮亮的待办"
tags:
  - index
  - todo
---

# ✅ 亮亮的待办

> Notion 为主数据源 · 最后同步：2026-05-19 20:34

---

## 📋 未完成（14 项）

| 标题 | 优先级 | 状态 | 类型 | 工作量 | 截止日期 |
|---|---|---|---|---|---|
| [【AssetNameMapping】相关代码合回 develop 分支](pages/AssetNameMapping-相关代码合回-develop-分支.md) | 🔴 High | 🔄 In progress | - | - | 2026-05-01 |
| [25???????????SaveData ??AssetNameMapping](pages/25-SaveData-AssetNameMapping.md) | 🔴 High | ⬜ Not started | - | - | 2026-05-25 |
| [SaveData 修复合入 SDK](pages/SaveData-修复合入-SDK.md) | 🔴 High | ⬜ Not started | - | - | - |
| [AssetNameMapping 及 SaveData 合入 SDK 和 ProjectT](pages/AssetNameMapping-及-SaveData-合入-SDK-和-ProjectT.md) | 🔴 High | ⬜ Not started | 💬 Feature request | Medium | - |
| [资产合入PT](pages/资产合入PT.md) | 🔴 High | ⬜ Not started | 💬 Feature request | Medium | - |
| [资产合入s19dev](pages/资产合入s19dev.md) | 🔴 High | ⬜ Not started | 💬 Feature request | Medium | - |
| [将develop_sdk分支上提交的资产迁移操作合并至develop_shadow分支以及develop分支](pages/将develop_sdk分支上提交的资产迁移操作合并至develop_shadow分支以及develop分支.md) | 🔴 High | 🔄 In progress | 💅 Polish | Medium | - |
| [savedata相关代码合入 dev 和 projectt 分支（需等前置依赖处理完毕合入后我再合入），并安排QA进行专项测试](pages/savedata相关代码合入-dev-和-projectt-分支-需等前置依赖处理完毕合入后我再合入-，并安排QA进行专项测试.md) | 🔴 High | 🔄 In progress | - | - | - |
| [上的功能需要测试启动耗时和基准内存](pages/上的功能需要测试启动耗时和基准内存.md) | 🟡 Medium | ⬜ Not started | - | - | - |
| [清理 tmr_sdk 下 Shell 仓库临时加入的文件](pages/清理-tmr_sdk-下-Shell-仓库临时加入的文件.md) | 🟡 Medium | ⬜ Not started | 💅 Polish | Small | - |
| [制作 skill：检查修复仓库内部对外部重定向器的依赖](pages/制作-skill：检查修复仓库内部对外部重定向器的依赖.md) | 🟡 Medium | ⬜ Not started | 💬 Feature request | Large | - |
| [提单冒烟 3C 相关功能](pages/提单冒烟-3C-相关功能.md) | 🟡 Medium | ⬜ Not started | 💬 Feature request | - | - |
| [推美术同学拆分 animation 仓库](pages/推美术同学拆分-animation-仓库.md) | 🟢 Low | ⬜ Not started | 💬 Feature request | - | - |
| [ini配置隔离方案](pages/ini配置隔离方案.md) | 🟢 Low | ⬜ Not started | - | - | - |

---

## ✅ 已完成（6 项）

| 标题 | 优先级 | 截止日期 |
|---|---|---|
| [和孙傲、王艺瑾打羽毛球](pages/和孙傲、王艺瑾打羽毛球.md) | 🔴 High | 2026-04-22 |
| [【元梦之星】【LiteProject】推进 Service 化，减少 LiteProject 对 System 仓库的全量依赖](pages/元梦之星-LiteProject-推进-Service-化，减少-LiteProject-对-System-仓库的全量依赖.md) | 🔴 High | 2026-04-27 |
| [【元梦之星】【LiteProject】解除 System 仓库对大厅仓库的依赖](pages/元梦之星-LiteProject-解除-System-仓库对大厅仓库的依赖.md) | 🔴 High | 2026-04-27 |
| [Fonts字体仓挪回到LetsGo仓库，并变成一个独立仓](pages/Fonts字体仓挪回到LetsGo仓库，并变成一个独立仓.md) | 🟢 Low | - |
| [分析连带迁入的SDK仓库 BP资产](pages/分析连带迁入的SDK仓库-BP资产.md) | 🟢 Low | - |
| [分析外部资产依赖 — 针对不同类型的资产找对应同学分析出方案](pages/分析外部资产依赖-—-针对不同类型的资产找对应同学分析出方案.md) | 🟢 Low | - |