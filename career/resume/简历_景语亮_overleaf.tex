  ...
  \item \textbf{大厅广场活动全流程交付}（需求拆分 $\rightarrow$ 前后端联调 $\rightarrow$ 上线热更）：
        ...
        \begin{itemize}[leftmargin=1.4em, topsep=1pt, label=$\circ$]
          \item \textbf{喜羊羊春节广场}：设计\textbf{可继承扩展的通用射击道具框架}（含可复用子弹对象池），制作射击变身道具；
                \textbf{平底锅道具}支持击打他人与击打自身两路命中逻辑，
                受击次数累积后逐级触发气泡提示、动画蓝图驱动的二头身、眩晕状态与 Mesh 压扁效果。
          \item \textbf{黄油小熊国庆广场}：设计并落地\textbf{通用可扩展、可配置的NPC/玩家走秀系统}——
                支持策划自由配置走秀\textbf{时间、点位、动画、服装、速度与各阶段时长}；走秀时替换动画蓝图的移动BlendSpace，替换专属T台走姿，
                并打通玩家上台排队、观众喝彩与舞台特效联动等完整互动流程。开发黄油小熊NPC，支持NPC邀请，寻路，双人动作互动以及特殊运镜等。
          \item \textbf{熊出没二周年广场}：DS管理NPC三态以及互斥，定时演出以服务器时间戳对齐进度，DS 重启与中途进场均从正确位置续播；
          打通完整的烹饪流程（镜头切换、LevelSequence、霸体控制）并实现五类菜品对应道具效果（加速跑、变形、喷气背包、泡泡升空、全身变黑）。
        \end{itemize}
